...
public class AdapterHandler<F extends AdapterData> {
    private final HashMap<Class, Consumers.TripleConsumer<WizardInfo, ? extends Target<?>, F>> map = new HashMap<>();

    public <E, T extends Target<E>> void onTarget(Class<T> tClass,  Consumers.TripleConsumer<WizardInfo, T, F> consumer) {
        map.put(tClass, consumer);
    }

    public <E, T extends Target<E>>  Consumers.TripleConsumer<WizardInfo, ? extends Target<?>, F> get(Class<T> key) {
